...
def resize_rgb(image_rgb: np.ndarray, width: int, height: int) -> np.ndarray:
    ...
    return np.asarray(
        Image.fromarray(image_rgb).resize((width, height), Image.Resampling.LANCZOS)
    )
...
def resample(
    image_rgb: np.ndarray, *, scale: float = 1.0, angle: float = 0.0
) -> np.ndarray:
    ...
    if scale != 1.0:
        w, h = img.size
        img = img.resize(
            (max(1, round(w * scale)), max(1, round(h * scale))), _RESAMPLE
        )
    if angle != 0.0:
